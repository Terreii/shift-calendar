...
class Shifts::Bosch66Test < ActiveSupport::TestCase
  test "returns the shifts of every group for a day" do
    ...
    assert_equal ({
      closed: false,
      shifts: [:free, :morning, :night, :free, :evening, :free]
    }), month.at(22)
    assert_equal ({
      closed: false,
      shifts: [:free, :evening, :free, :morning, :night, :free]
    }), month.at(23)

    month2 = Shifts::Bosch66.new year: 2023, month: 12
    assert_equal ({
      closed: false,
      shifts: [:morning, :free, :free, :night, :free, :evening]
    }), month2.at(8)
    assert_equal ({
      closed: true,
      shifts: [:night, :free, :evening, :free, :morning, :free]
    }), month2.at(24)
  end
  ...
  test "should have closing days" do
    month = Shifts::Bosch66.new year: 2022, month: 12
    assert month.at(Faker::Number.within(range: 24..26))[:closed], "X-mas"
    
    month = Shifts::Bosch66.new year: 2023, month: 4
    assert month.at(Faker::Number.within(range: 7..10))[:closed], "Easter 2023"

    month = Shifts::Bosch66.new year: 2022, month: 4
